...
import { isCrmEnabled, isCrmOnlyEdition, isExpenseOnlyEdition } from "./moduleConfig";
...
const allProtectedRoutes = [
  ...
  },
  {
    ...
    breadcrumbs: ({ params, state }) => {
      const client = state?.clients?.clients?.find(
        (item) => String(item.id) === params.clientId
      );

      return [
        { label: "Sales" },
        { label: "Clients", href: "/clients" },
        { label: client?.company_name || `Client ${params.clientId} Ledger`, current: true },
      ];
    },
  },
  {
  ...
  },
  {
    ...
    breadcrumbs: [
      { label: "Purchase" },
      { label: "Vendor Payments", current: true },
    ],
  },
  {
...

  {
    ...
    breadcrumbs: [
      { label: "Platform" },
      { label: "Platform Dashboard", current: true },
    ],
  },
...
const routeMatchesEdition = (route) => {
  if (!isCrmEnabled && route.group === "crm") return false;
  if (isCrmOnlyEdition) return route.group === "crm";
  if (isExpenseOnlyEdition) return !route.group;
  return true;
};

export const protectedRoutes = allProtectedRoutes.filter(routeMatchesEdition);
